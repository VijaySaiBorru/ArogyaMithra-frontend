...
const EditTreatmentDoctor = ({ appointmentId }) => {
  ...
  const [updateTreatment] = useUpdateAppointmentDetailsMutation();
  ...
  // Populate the form with existing treatment data if available
  useEffect(() => {
    if (existingTreatment) {
        setFormData({
            appointment_id: existingTreatment.appointment_id,
            diagnosis: existingTreatment.diagnosis || "",
            treatment_plan: existingTreatment.treatment_plan || "",
            medications: existingTreatment.medications?.length
              ? existingTreatment.medications.map((med) => ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (existingTreatment) {
        ...
        // Add new treatment if none exists
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto bg-white rounded shadow h-[calc(100vh-80px)] flex flex-col overflow-hidden mt-2">
      <div className="p-6 overflow-y-auto flex-grow">
        <h2 className="text-2xl font-semibold mb-4 text-blue-700">Edit Treatment Data</h2>
      ...
      <div className="p-6 border-t bg-gray-50">
        ...
        >
          {isLoading ? "Submitting..." : "Update Treatment"}
        </button>
      </div>
    </div>
  );
...
export default EditTreatmentDoctor;
